...
describe("session-collector", () => {
    it("should log user message as input", () => {
        ...
        handleChatMessage({ base: BASE, input: input, output: output });
    ...
    it("should log assistant message as output with thinking", () => {
        ...
        assert.equal(records[0].input, undefined);
    });
    ...
    it("should skip part without time.end", () => {
        ...
        const records = readJSONL(BASE, SESSION_LOGS);
        assert.equal(records.length, 0, "no records should be flushed for part without time.end");
    });
