...
const CONFIG_ENABLED = { includeThinking: true };
const CONFIG_DISABLED = { includeThinking: false };
after(() => {
    try {
...
describe("session-collector", () => {
    it("should log user message as input", () => {
        ...
        handleChatMessage(BASE, input, output);
    ...
    it("should log assistant message as output with thinking", () => {
        ...
        handleChatMessage(BASE, input, output, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, CONFIG_ENABLED);
        const records = readJSONL(BASE, "session-logs");
    ...
    it("should include root_dir in chat message records", () => {
        ...
        handleChatMessage(BASE, input, output, undefined, "/test/project");
        const records = readJSONL(BASE, "session-logs");
        const last = records[records.length - 1];
    ...
    it("should include username in chat message records", () => {
        ...
        const output = { message: { role: "user" }, parts: [{ type: "text", text: "hello" }] };
        handleChatMessage(BASE, input, output, undefined, undefined, "testuser");
        const records = readJSONL(BASE, "session-logs");
        const last = records[records.length - 1];
    ...
    it("should include model info and opencode version in chat message records", () => {
        ...
        handleChatMessage(BASE, input, output, undefined, undefined, undefined, "anthropic", "claude-3", "1.15.0");
        const records = readJSONL(BASE, "session-logs");
        const last = records[records.length - 1];
    ...
    it("should include project_id, git_branch and skills in chat message records", () => {
        ...
        handleChatMessage(BASE, input, output, undefined, undefined, undefined, "anthropic", "claude-3", "1.15.0", "proj-1", "main", ["test-driven-development", "writing-plans"]);
        const records = readJSONL(BASE, "session-logs");
        const last = records[records.length - 1];
        assert.equal(last.project_id, "proj-1");
        assert.equal(last.git_branch, "main");
    ...
    it("should not include skills for user messages", () => {
        ...
        const output = { message: { role: "user" }, parts: [{ type: "text", text: "hello" }] };
        handleChatMessage(BASE, input, output, undefined, undefined, undefined, undefined, undefined, undefined, "proj-1", "main", ["test-driven-development"]);
        const records = readJSONL(BASE, "session-logs");
        const last = records[records.length - 1];
        assert.equal(last.project_id, "proj-1");
        assert.equal(last.git_branch, "main");
...
describe("session-collector with includeThinking", () => {
    ...
    it("should include reasoning when includeThinking is true", () => {
        handleChatMessage(BASE, { sessionID: "test-session", agent: "test-agent" }, {
            message: { role: "assistant" },
            parts: [
                { type: "text", text: "Hello world" },
                { type: "reasoning", text: "Thinking process..." }
            ]
        }, "2026-01-01T00:00:00.000Z", undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, CONFIG_ENABLED);
        const records = readJSONL(BASE, "session-logs");
    ...
    it("should exclude reasoning when includeThinking is false", () => {
        handleChatMessage(BASE, { sessionID: "test-session-2", agent: "test-agent" }, {
            message: { role: "assistant" },
            parts: [
                { type: "text", text: "Hello again" },
                { type: "reasoning", text: "More thinking" }
            ]
        }, "2026-01-01T00:01:00.000Z", undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, CONFIG_DISABLED);
        const records = readJSONL(BASE, "session-logs");
    ...
    it("should default to false when config is undefined", () => {
        handleChatMessage(BASE, { sessionID: "test-session-3", agent: "test-agent" }, {
            message: { role: "assistant" },
            parts: [
                { type: "text", text: "Test" },
                { type: "reasoning", text: "Should be excluded" }
            ]
        }, "2026-01-01T00:02:00.000Z", undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined);
        const records = readJSONL(BASE, "session-logs");
