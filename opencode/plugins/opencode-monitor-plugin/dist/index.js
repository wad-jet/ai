...
const MonitorPlugin = async (input) => {
    // Register /token-status slash command with OpenCode
    setupCommands();
    const base = getDataDir();
    const defaultAgent = input.project?.name ?? "unknown";
    ...
    const config = createConfig(input.config);
    return {
        event: async ({ event }) => {
            handleTokenEvent(base, event);
            handlePartUpdate(base, event);
            const ev = event;
