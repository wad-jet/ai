import { queryTokenSummary, queryAgentBreakdown, queryDailyBreakdown } from "../../queries/token-query.js";
import { getDataDir } from "../../paths.js";
export function runTokenStatusCLI(args) {
...
export function formatCLITokenTable(summary) {
...
export function formatDailyTable(rows) {
    if (rows.length === 0)
        return "";
    const lines = [];
    ...
    lines.push("  " + "─".repeat(80));
    lines.push(`  Total:    ${String(totalHits).padStart(5)} ${totalInput.toLocaleString().padStart(10)} ${totalOutput.toLocaleString().padStart(10)} ${totalReasoning.toLocaleString().padStart(10)} ${totalCacheR.toLocaleString().padStart(8)} ${totalCacheW.toLocaleString().padStart(8)} ${`$${totalCost.toFixed(4)}`.padStart(9)}`);
    return lines.join("\n");
}
export function formatAgentTable(agents, noData) {
    if (noData || agents.length === 0)
        return "";
    const lines = [];
