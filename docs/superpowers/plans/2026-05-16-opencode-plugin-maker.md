# opencode-plugin-maker Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a comprehensive SKILL.md reference for developing opencode plugins.

**Architecture:** Single-file SKILL.md with combined reference + process content. All API types, hooks, examples, and workflow documented inline. No external files needed.

**Tech Stack:** Markdown, TypeScript (for type definitions in examples)

---

### Task 1: Create SKILL.md with full opencode plugin reference

**Files:**
- Create: `.opencode/skills/opencode-plugin-maker/SKILL.md`

- [ ] **Step 1: Create the directory and YAML frontmatter**

Run:
```bash
mkdir -p .opencode/skills/opencode-plugin-maker
```
...
- [ ] **Step 2: Write Overview + Architecture section**

Content:
- What is a plugin: JS/TS module that exports a function returning hooks
- Load order: global config → project config → global plugins dir → project plugins dir
- Two types: local files and npm packages
- All hooks run in sequence; duplicate names loaded separately
...
Duplicate npm packages (same name + version) load once. A local and npm plugin with similar names both load.
...
- [ ] **Step 3: Write Quick Reference Table**
...
| `session.*` events | Session CRUD | Track session lifecycle |
...
- [ ] **Step 4: Write Plugin API Reference — Types & Hooks**
...
- [ ] **Step 5: Write Custom Tools API Reference**
...
- [ ] **Step 6: Write Auth & Provider API Reference**
...
- [ ] **Step 7: Write Setup & Installation section**
...
- [ ] **Step 8: Write Development Workflow section**
...
- [ ] **Step 10: Write Common Patterns & Pitfalls**
...
### Name Collisions
- Local plugin and npm plugin with similar names: both load separately
- Custom tool with same name as built-in: custom wins
- **Fix:** Use unique, descriptive names

### Async Error Handling
- Hooks are async — unhandled rejections crash the plugin
- **Fix:** Always wrap in try/catch or use `.catch()`

### Forgetting `noReply`
- Calling `client.session.prompt()` without `noReply: true` triggers an AI response
- **Fix:** Always set `noReply: true` when injecting context

### Plugin Load Order Assumptions
- Don't assume your plugin loads before/after another
- All hooks run in sequence but order across plugins is not guaranteed
- **Fix:** Make plugins idempotent and independent

### Blocking the Event Loop
- Heavy sync operations in hooks block opencode
- **Fix:** Use async/await, offload heavy work

### Overriding Tool Definitions
- `tool.definition` hook modifies what LLM sees about a tool
- Changes affect all subsequent calls
- **Fix:** Only modify specific toolID, avoid blanket changes
```

- [ ] **Step 11: Write SDK Integration Reference**
...
- [ ] **Step 12: Verify the file reads correctly and is complete**

Run: `wc -l .opencode/skills/opencode-plugin-maker/SKILL.md`
Expected: > 400 lines of comprehensive content

- [ ] **Step 13: Commit the skill**

```bash
git add .opencode/skills/opencode-plugin-maker/SKILL.md
git commit -m "feat(skills): add opencode-plugin-maker skill with full plugin API reference"
```
